...
$scope.listAccount = function(){
  Accounts.listAccounts(function (err, data){
    ...
    $scope.keepIngredient = function (ingredient) {
      ...
      $scope.selectedingregients.push($scope.keep_ings.iID);
    ...
    $scope.keepStep = function (step) {
      ...
      $scope.selectedsteps.push($scope.keep_steps.sID);
    ...
    $scope.addRecipe = function () {
      if (
        $scope.categoryList.selectedOption.categoryTitle != undefined &&
        ...
        RecipeList.postRecipe(function (data, err) {
          if (!err) {
            RecipeList.getRecipes(function (err, data) {
              if (!err) {
                $scope.categoryList = data;
    ...
    $scope.updateRecipe = function (selectedsteps, selectedingregients) {
      if (selectedsteps != null || selectedingregients != null) {
        ...
        while (s < selectedsteps.length) {
          ...
          RecipeList.deleteSteps(sid, function (err, data) {
            if (!err) {
              $scope.msg = "success";
            }
          });
          s++;
          if (s > selectedsteps.length) break;
        }
        ...
        while (i < selectedingregients.length) {
          ...
          RecipeList.deleteIngs(iid, function (err, data) {
            if (!err) {
              $scope.msg = "success";
            }
          });
          i++;
          if (i > selectedingregients.length) break;
        }
      } else {
        RecipeList.putRecipe($scope.selectedRecipe, function (err, data) {
          if (!err) {
    ...
    $scope.deleteRStep = function (selectedsteps) {
      if (selectedsteps != null) {
        ...
        while (i < selectedsteps.length) {
          ...
          RecipeList.deleteSteps(id, function (err, data) {
            if (!err) {
              $scope.msg = "success";
            }
          });
          i++;
          if (i > selectedsteps.length) break;
        }
      } else {
        alert("please complete you recipe form!");
      }
    ...
    $scope.deleteRecipe = function (id) {
      var id;
      id = $scope.id;
      RecipeList.deleteRecipe(id, function (err, data) {
        if (!err) {
          $scope.msg = "success";
        }
        RecipeList.getRecipes(function (err, data) {
